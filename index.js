...
if (!SUPABASE_URL || !SUPABASE_KEY) {
    console.error('[SUPABASE ERROR] Λείπουν τα SUPABASE_URL ή SUPABASE_KEY από τα περιβάλλοντα (Secrets)!');
}
...
function getGreekTime() {
    ...
    return {
        ...
        month: now.getMonth(), // Ιανουάριος είναι το 0
        date: now.getDate(),
        day: now.getDay(), // Κυριακή είναι 0
        hour: now.getHours(),
...
// ΧΡΙΣΤΟΥΓΕΝΝΑ: 18 Νοεμβρίου - 31 Ιανουαρίου (Μήνες στο JS: 10 = Νοέμβριος, 11 = Δεκέμβριος, 0 = Ιανουάριος)
...
function getOrthodoxEasterDate(year) {
    ...
    easterUTC.setUTCDate(easterUTC.getUTCDate() + 13); // Μετατροπή σε Γρηγοριανό
    return easterUTC;
}

// ΠΑΣΧΑ: Από Μ. Δευτέρα έως Κυριακή του Θωμά
...
function getRequiredGenre() {
    ...
    // Καθημερινές - Βραδινές / Πρωινές ώρες
    if (h >= 20 || h < 2) return 'MIX';  

    // Καθημερινές
    if (d === 1 || d === 3 || d === 5) { // Δευτέρα, Τετάρτη, Παρασκευή
        if (h >= 2 && h < 7) return 'B';  
...
// Συνάρτηση για να βρίσκει τα tags, π.χ. αρχεία που ξεκινούν με (B) ή (Π)
function hasTag(filename, ...variants) {
    return variants.some(v => filename.startsWith(`(${v})`));
}

// Αλγόριθμος τυχαίας ανακατάτευσης (Fisher-Yates) για σωστό Shuffle!
...
const FONT_ARG = fs.existsSync(FONT_PATH) ? `fontfile='${FONT_PATH}':` : '';
...
async function selectNextFile() {
    ...
    if (liveRequest) {  
        let displayTitle = liveRequest.filename.replace(/^\([A-ZΠΛΖX]+\)\s*/i, '').replace('.mp3', '').replace(/_/g, ' ');  
        return {  
    ...
    // Επιλογή κατηγορίας βάση κανόνων
    if (genre === 'B') {  
        ...
        // ΠΛΕΟΝ ΠΑΙΖΕΙ ΑΠΟ ΟΛΟ ΤΟ NORMAL POOL, ΔΗΛΑΔΗ ΚΑΝΟΝΙΚΟ MIX
    ...
    // Fallback αν αδειάσει τελείως ο φάκελος της συγκεκριμένης κατηγορίας
    ...
    // LOGIC: Να μην παίζουν τα ίδια τραγούδια (No repeat)
    let availableFiles = filteredFiles.filter(f => !globalPlayedSongs.includes(f));  
    
    // Αν έπαιξαν ΟΛΑ τα τραγούδια της κατηγορίας, μηδενίζουμε τη λίστα της κατηγορίας
    ...
    if (availableNewFiles.length > 0) {  
        ...
        // LOGIC: Τυχαία επιλογή (Shuffle)
        shuffleArray(availableFiles);  
        randomFile = availableFiles[0];  
    }  

    // Προσθήκη στα "παιγμένα" για να μην ξαναπαίξει σύντομα
    globalPlayedSongs.push(randomFile);  
    
    // Αφαίρεση του Tag (π.χ. (B), (Π)) από τον τίτλο που φαίνεται στην οθόνη
    let displayTitle = randomFile.replace(/^\([A-ZΠΛΖX]+\)\s*/i, '').replace('.mp3', '').replace(/_/g, ' ');  
    return { file: randomFile, title: displayTitle, genreLabel: genreLabel, isSong: true };
}

// ... Το κομμάτι με τα buildNewYearCountdownFilters, athensTargetDate, secondsFromNowTo παραμένει το ίδιο 
